...
export const useCreatePost = () => {
  ...
  const [isOpenActionConfirmation, setIsOpenActionConfirmation] =
    useState(false);
  const [isDisableInput, setIsDisableInput] = useState(false);

  const [createPostImage, { isLoading }] =
    postsApi.useCreatePostImageMutation();
  ...
  const uploadImageForPost = async (file: File | null) => {
    if (file) {
      ...
      setUploadIds((imagesId) => [
        ...imagesId,
        { uploadId: res.images[0].uploadId },
      ]);
    }
  };

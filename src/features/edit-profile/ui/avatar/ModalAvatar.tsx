...
export const ModalAvatar = ({
  ...
  const { crop, previewImgRef, saveCroppedImage, setCrop } =
    useCropImage(updateAvatar);
  ...
  return (
    ...
    >
      {error && (
        <Alert
          className={
            "static left-0 right-0 !mb-0 md:absolute md:left-4 md:right-4"
          }
          message={error}
          onClose={() => setError("")}
          purpose={"alert"}
      ...
      <div className={"w-full items-end"}>
        ...
        {!imageUrl && (
          <div
            className={
              "w-full flex flex-col justify-center items-center gap-9 md:gap-14"
            }
          >
            <DragAndDropInput
            ...
            <Button
              className={"w-56 bottom-0 mb-9 h-12 md:h-9 md:mb-[84px]"}
              onClick={uploadImage}
            >
              {t.selectPhoto}
            </Button>

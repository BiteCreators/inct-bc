...
export const useSubmitPayment = ({
  ...
  const subscriptionType = useAppSelector(
    paymentsSlice.selectors.selectNewSubscriptionType,
  );
  ...
  const getPaymentDataByType = (desc: TYPE_DESCRIPTIONS | null) => {
    if (subscriptionTypes) {
      return subscriptionTypes.data.find(
        (type) => type.typeDescription === desc,
      );
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    const { data, error, success } =
      createSubscriptionSchema(t).safeParse(submitData);

...
export const ProfileHeader = ({ profile }: Props) => {
  ...
  return (
    <>
      <div
        className={
          "flex items-center sm:items-start gap-5 sm:gap-7 md:!gap-9 mb-2 sm:mb-12"
        }
      >
        ...
        <div className={"flex-1 text-white"}>
          <div className={"hidden justify-between mb-5 sm:flex gap-5"}>
            ...
            {isCurrentUserProfile && (
              <Button
                ...
                variant={"secondary"}
              >
                <Link href={`/profile/${profile.id}/settings`}>
          ...
          <AboutUser
            className={"hidden sm:flex text-left"}
      ...
      <div>
        <Typography
          className={"sm:hidden font-weight700 mb-3"}
          variant={"regular-text"}
        >
          {profile.userName}
        </Typography>
        <AboutUser
          className={"flex sm:hidden text-left text-sm"}
          text={profile.aboutMe || ""}
        />
      </div>
    </>
